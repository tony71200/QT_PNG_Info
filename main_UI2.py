...
from PyQt5.QtGui import QDragEnterEvent, QDropEvent, QFontMetrics, QIcon, QPixmap, QTextCursor
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.lbl_folder.setStyleSheet(
            "font-weight: bold; color: #FFFFFF; background-color: #333333; padding: 5px;"
        )
        ...
        self.btn_add_refresh = QPushButton("Fresh")
        self.btn_add_refresh.clicked.connect(self.refresh_images_in_folder)

        filter_row = QHBoxLayout()
        filter_row.addWidget(self.cbo_add_image, 1)
        filter_row.addWidget(self.btn_add_refresh)
        ...
        add_layout.addLayout(filter_row)
    ...
    def refresh_images_in_folder(self):
        folder = self.folder_path
        if (not folder) and self.current_image_path:
            folder = os.path.dirname(self.current_image_path)
            self.folder_path = folder
            self.lbl_folder.setText(folder)

        if not folder or not os.path.isdir(folder):
            self.statusBar().showMessage("Chưa có thư mục hợp lệ để refresh")
            return

        current = self.current_image_path
        self.load_images_from_folder(folder)
        if current and os.path.exists(current):
            self._select_image_by_path(current)

        self.statusBar().showMessage("Đã cập nhật danh sách ảnh")
    ...
    def _select_image_by_path(self, path: str):
        for i in range(self.list_images.count()):
            item = self.list_images.item(i)
            if item and item.data(Qt.UserRole) == path and (not item.isHidden()):
                self.list_images.setCurrentRow(i)
                self.display_selected_image(item)
                return
                ...
                if meta_text is None and hasattr(img, "_getexif") and img._getexif():
                    ...
                    if exif:
                        for tag, value in exif.items():
                            ...
                            if decoded in ["UserComment", "ImageDescription", "XPComment", "XPSubject"]:
                                if isinstance(value, str) and len(value) > 10:
                                    meta_text = value
                                    meta_key = decoded
                                    break
    ...
    def parse_block(self, block):
        ...
        meta = OrderedDict()
        meta["Positive prompt"] = " ".join(positive_lines).strip()
        meta["Negative prompt"] = " ".join(negative_lines).strip()
    ...
    def replace_all_in_prompts(self):
        ...
        if total == 0:
            self.statusBar().showMessage("Không tìm thấy ảnh hợp lệ trong filter hiện tại")
            return
        ...
        def on_finished(replacements, changed_paths):
            ...
            if replacements:
                ...
                self.statusBar().showMessage("Không có ảnh nào thay đổi")
    ...
    def save_prompt_file(self):
        ...
        self.statusBar().showMessage("Đã lưu metadata")
    ...
    def _update_add_meta_tab(self, meta_for_tree, meta_key, meta_text):
        ...
        if not negative and (not self._is_meaningful_metadata(meta_for_tree)):
            negative = self.default_negative_prompt
        ...
        if is_png:
            ...
            self.lbl_add_png_status.setText("PNG detected.")
        else:
            self.lbl_add_png_status.setStyleSheet("font-weight: bold; color: #d11b1b;")
            self.lbl_add_png_status.setText("Not PNG: will convert to PNG and delete the old file.")

        has_embedded = bool(meta_key and meta_text and str(meta_text).strip())
        missing_positive = not bool(positive.strip())
        enable = (not is_png) or (not has_embedded) or missing_positive
    ...
    def _build_stable_diffusion_metadata_text(self, positive: str, negative: str, size: str) -> str:
        ...
        extras_line = ", ".join(
            [f"{k}: {v}" for k, v in extras_defaults.items() if v is not None and str(v).strip()]
        )
        if extras_line:
        ...
        try:
            ...
            if dst_path != src_path:
                try:
                    ...
                    pass
                self.load_images_from_folder(os.path.dirname(dst_path))
    ...
    def handle_dropped_images(self, image_paths):
        ...
        image_extensions = [".jpg", ".jpeg", ".png", ".bmp", ".webp"]
        all_images = sorted(
            [f for f in os.listdir(folder) if any(f.lower().endswith(ext) for ext in image_extensions)]
        )
...
if __name__ == "__main__":
    ...
    try:
        with open("ElegantDark.qss", "r", encoding="utf-8") as f:
